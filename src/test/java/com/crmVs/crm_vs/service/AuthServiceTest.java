...
class AuthServiceTest {
    ...
    @BeforeEach
    void setUp() {
        // Inyectamos las properties del superadmin
    ...
    void login_emailIncorrecto_noDevuelveInfoSiPasswordEsCorrecta() {
        // Verificamos que el mensaje de error es el mismo
        // sin importar si falla el email o la password
        // para no dar información al atacante
        when(userRepository.findByEmail("noexiste@empresa.com"))
                .thenReturn(Optional.empty());
    ...
    void login_siempreLlamaAlRepositorioPorEmail() {
        ...
        // Verificamos que la búsqueda se hizo por email
        // y solo una vez
    ...
    void login_passwordNuncaSeVerificaSiEmailNoExiste() {
        ...
        // Si el email no existe, nunca debería llamarse al passwordEncoder
        // porque sería un gasto innecesario de CPU (BCrypt es lento adrede)
        verify(passwordEncoder, never()).matches(any(), any());
    }

    // ─────────────────────────────────────────────────────────
    // Tests de admin login
    // ─────────────────────────────────────────────────────────
    ...
    void adminLogin_nuncaConsultaLaBaseDeDatos() {
        ...
        // El superadmin no existe en la BD, nunca debería consultarla
    ...
    void adminLogin_credencialesCorrectas_llamaAlGeneradorDeTokenAdmin() {
        ...
        // Verifica que usa generarTokenAdmin, no generarToken
        // (son tokens distintos con claims distintos)
